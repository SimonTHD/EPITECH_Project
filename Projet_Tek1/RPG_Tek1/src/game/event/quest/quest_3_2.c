...
{
    if (debug_mode)
        my_putstr("declencher quete 3_2\n");
    quest_t *new_quest = malloc(sizeof(quest_t));
    new_quest->quest_id = 532;
    new_quest->title = strdup(quest_3_2_title);
    new_quest->description = strdup(quest_3_2_describe);
    new_quest->is_completed = false;
    new_quest->action_id = 13;
    new_quest->next = NULL;
    new_quest->reward = &reward_3_2;
